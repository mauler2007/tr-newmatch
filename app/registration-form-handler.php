...
function someFunc() {
  ...
  $country = $_POST['country']; // [код_страны]​- двубуквенный код страны
  $currency = $_POST['cyrrency']; // [код_валюты]​- трёхбуквенный код валюты
  $send_reg_data = '1'; // [send_reg_data]​- отправлять ли письмо с рег данными на почту или нет, по умолчанию не отправляется
  $tag = $_POST['tag']; //​ [tag]​- партнёрский тэг
  $promocode = ''; // [promocode] -​партнёрский промокод
  $bonus_choice = ''; // [bonus_choice]​- 1 на спорт / 2 на казино / 3 отказаться
  ...
  $sign = md5( $secret . $id . $phone . $email); // ​[подпись]​ = md5(secret + id + ([email] или [phone]))
